...
{
    // Forziamo il risultato: facciamo credere a Doom di aver trovato il file
    printf("Zoe Hack: Bypassing SD search, using WAD from SDRAM at 0x91000000\n");

    // 1. Impostiamo il nome del file (servirà a W_AddFile che abbiamo già modificato)
    char *result = "doom1.wad";

    // 2. Identifichiamo la missione manualmente. 
    // Per DOOM1.WAD (Shareware), la missione è 'doom'.
    *mission = doom; 

    // 3. Restituiamo il nome per far felice il chiamante
    return result;
}
/*char *D_FindIWAD(int mask, GameMission_t *mission)
{
